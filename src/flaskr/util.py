...
def get_post(id, check_author=True):
    try:
        apost = model_to_dict(PostDB.select(PostDB.id, PostDB.title, PostDB.body, PostDB.created, PostDB.author_id).where(PostDB.id == id).get())
...
def get_index_info():
    ...
    for apost in allposts:
        ...
        this_user = model_to_dict(UserDB.select(UserDB.username).where(UserDB.id == dct_apost['author']['id']).get())
